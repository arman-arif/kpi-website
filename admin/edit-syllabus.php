<?php
	...
	$rl_info = $odb->getDataById("syllabus", $id)->fetch_object();
	...
	if (isset($_POST['dept'])){
		...
		if (!empty($_FILES['file']['name'])){
			$file= $oup->uploadFile($_FILES, "file", "syllabus");
			if ($file != false){
				$odl->deleteFile($rl_info->file, "syllabus");
			}
		}
		...
		if ($file != false){
			$odb->updateData('syllabus', $data, $id);
			header("location: view-syllabus-list.php");
		}
	}
