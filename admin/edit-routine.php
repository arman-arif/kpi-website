<?php
	...
	$rl_info = $odb->getDataById("routine", $id)->fetch_object();
	...
	if (isset($_POST['dept'])){
		...
		if (!empty($_FILES['file']['name'])){
			...
			if ($file != false){
				$odl->deleteFile($rl_info->file, "routine");
			}
		}
		...
		if ($file != false){
			$odb->updateData('routine', $data, $id);
								...
								<div class="form-group row">
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-control" name="shift" required>
                                            ...
                                            <option<?= (isset($data['shift']) && $data['shift'] == "1st") ? " selected":(($rl_info->shift == "1st")?" selected":"") ?> value="1st">1st Shift</option>
                                            <option<?= (isset($data['shift']) && $data['shift'] == "2nd") ? " selected":(($rl_info->shift == "2nd")?" selected":"") ?> value="2nd">2nd Shift</option>
                                        </select>
                                    </div>
                                    <div class="col-md-6">
                                        <label>Regulation</label>
                                        <input type="number" name="regulation" class="form-control" required  value="<?= isset($data['regulation'])?$data['regulation']:$rl_info->regulation ?>">
                                    </div>
								</div>
